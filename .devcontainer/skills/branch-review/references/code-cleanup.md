...
4. **Over-defensive checks** — validation only when impossibility is structurally preserved by a refined representation, controlled constructor, or trusted enforced boundary, or when an earlier check dominates every downstream use and the validated value and invariant cannot change between check and use. Current caller validation alone is insufficient; if removing the check requires a new representation or architectural refactor, drop it from cleanup.
