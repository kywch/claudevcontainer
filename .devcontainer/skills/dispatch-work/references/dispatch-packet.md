...
- seed-declared `area` set, optional `support_area` set, and resulting `repo_edit_roots`. Prefer the
  ...
  from label spelling or language name. Add `support_area` roots to
  `repo_edit_roots` only for gate, harness, wrapper, or report wiring needed to
  prove this seed; they do not own product behavior.
