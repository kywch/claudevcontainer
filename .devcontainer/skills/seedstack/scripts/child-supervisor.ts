...
function validateManageQueueProposals(result: ChildResult, seed: string): void {
  ...
  for (const [index, proposal] of proposals.entries()) {
    ...
    if (stringArray(proposal.expected_preconditions).length === 0) throw new Error(`manage child proposal ${index} expected_preconditions required`);
  }
    ...
    proposed_queue_operations: [
      {
        ...
        expected_preconditions: ["timeout result generated by supervisor"],
      },
    ],
      ...
      proposed_queue_operations: [
        {
          ...
          expected_preconditions: ["seed-test-legacy is still open"],
          details: {},
        },
      ],
    });
...
setTimeout(() => {
  writeFileSync(result, JSON.stringify({
    ...
    proposed_queue_operations: [{
      ...
      expected_preconditions: ["blocked result is not applied"]
    }],
    blocked_reason: "self_test_done"
  }, null, 2) + "\\n");
}, 20);
